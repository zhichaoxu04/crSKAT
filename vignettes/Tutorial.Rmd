...
Interval-censored data is a subset of failure time or time-to-event data, commonly seen in modern genetic datasets like the UK Biobank (UKB) and the St. Jude Lifetime Cohort Study (SJLIFE). While many are obtained with right-censored data—a specific type of interval-censored data—the exact time for interval-censored data remains elusive, known only to lie within a defined range or interval, such as between two specific time points or up to infinity.

In the UK Biobank (UKB), participants' follow-up information was sourced from questionnaires conducted during visits, later integrated with their medical histories. This method of data collection poses challenges in generating longitudinal data with precise timestamps for phenome-wide diagnoses. For some conditions, the onset is deduced to have occurred within an interval between two assessments, or prior to the first assessment, rather than pinpointing an exact time, leading to interval-censored failure times.

Competing risks emerge when an individual could potentially encounter multiple events leading to failure, with these events vying against each other. In essence, one event can alter or preclude the occurrence of another. For example, within a study's duration, certain participants might die from unrelated causes, making them ineligible to experience the event under study or to persist until the study's conclusion.

While there are some ad-hoc approaches to manage interval-censored data—like translating event occurrences into binary outcomes for logistic regression methods, or estimating event times based on interval midpoints for right-censored survival analysis—these may not suffice when competing outcomes are in play. In such scenarios, leveraging interval-censored techniques that accommodate these competing risks often yields superior results, such as a better control of  over Type I error rates or enhanced statistical power.
...
- Make sure that all the required packages have been installed or updated. Here are some of the required packages:
  ...
  - [cmprsk](https://cran.r-project.org/web/packages/cmprsk/index.html): Estimation, testing and regression modeling of subdistribution functions in competing risks.
  - [Rcpp](https://cran.r-project.org/web/packages/Rcpp/index.html) (>= 0.11.3): Provides R functions as well as C++ classes which offer a seamless integration of R and C++.
...
## Notes:
Questions or novel applications? Please let me know! Contact information can be found in the Github 
